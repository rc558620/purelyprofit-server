...
## 更新摘要
**变更内容**
- **重大重构**：从多合作伙伴架构迁移到单合作伙伴架构，简化了订单处理流程
- **自动修复机制**：应用启动时自动修复历史未充值推广记录，确保数据一致性
- **增强豆扣除功能**：使用原子更新和乐观锁模式，提升并发安全性和数据一致性
- **幂等保护**：通过 hasCharged 标记实现推广奖励的幂等发放，避免重复奖励
...
## 简介
本文件系统性阐述平台会员管理功能，覆盖平台会员体系、会员订单管理、合作伙伴管理、权益与收益分配、统计分析与生命周期管理等核心能力。**重大更新**：系统已从多合作伙伴架构重构为单合作伙伴架构，实现了简化的订单处理流程。新增应用启动时的自动推广记录修复机制，以及基于原子更新和乐观锁模式的增强豆扣除功能，大幅提升系统的并发安全性和数据一致性。文档以代码级实现为依据，结合序列图、类图与流程图，帮助开发者与产品人员快速理解业务闭环与技术实现，并提供 API 使用示例与集成指引。
...
- 订单服务：**重大更新** - 实现单合作伙伴架构下的简化订单处理，支持应用启动时的自动推广记录修复
- 读取服务：中心页、订单列表、合作伙伴档案等只读聚合
- 账本服务：积分流水、可用积分概览、到期计算
- 合作伙伴服务：**重大更新** - 适配单合作伙伴架构，简化审核流程
- 促销领域：邀请奖励、结算、统计，**新增** - 支持幂等保护和自动修复
...
## 架构总览
平台会员管理采用"控制器-服务-领域-查询-数据模型"的分层架构。**重大更新**：重构为单合作伙伴架构，简化了业务流程和数据模型。控制器负责路由与鉴权，服务编排业务，领域函数处理规则，查询封装数据库访问，数据模型由 Prisma 统一管理。
...
### 1) 会员购买流程与订单处理
**重大更新**：重构为单合作伙伴架构，简化订单处理流程，增强并发安全性

- **新流程要点**
  - 校验门店主账号权限
  - 解析套餐计划与有效期
  - 计算支付金额、积分/纯利豆抵扣上限与赠送积分
  - **新增** - 使用原子更新进行纯利豆扣减，防止并发问题
  - 创建订单、更新会员档案（生效期、积分余额）、写入积分日志
  - **新增** - 异步触发推广奖励发放，支持幂等保护
...
- **关键路径**
  ...
  - **新增** - 原子更新合伙人豆余额（乐观锁模式）
  - 更新会员档案与积分流水
  - **新增** - 异步推广奖励发放（hasCharged 标记作为并发锁）
  - 返回订单与最新会员信息
...
### 3) 合作伙伴管理与审核
**重大更新**：适配单合作伙伴架构，简化审核流程

- **新流程要点**
  - 申请与档案：构建当前申请与历史申请列表，映射跟进记录
  - **新增** - 单合作伙伴查找：每个门店只有一个当前合作伙伴
  - 审核兼容：保留旧版审核字段与流程兼容
  - 档案聚合：按门店聚合合作伙伴、促销与申请记录，输出可展示档案

- **关键改进**
  - `findCurrentStorePartner` 函数改为返回单一合作伙伴
  - 简化合作伙伴状态管理逻辑
  - 保持向后兼容性
...
Note over PD : 单合作伙伴逻辑
PD->>DB : "findFirst(status='approved')"
DB-->>PD : "返回唯一合作伙伴"
PD-->>PP : "返回聚合结果"
PP-->>PS : "返回档案"
PS-->>C : "返回响应"
...
- **新特性**
  ...
  - **新增** - 原子豆扣除：使用 updateMany + gte 条件实现乐观锁
  - **新增** - 并发安全：防止超扣和竞态条件
...
- **新特性**
  ...
  - **新增** - 应用启动自动修复：扫描未充值的推广记录并补发奖励
  - **新增** - 幂等保护：hasCharged 标记防止重复发放
  - **新增** - 并发安全：updateMany + hasCharged=false 作为并发锁
...
### 7) 数据模型与关系
平台会员相关模型包括：会员档案、订单、积分日志、合作伙伴、申请与跟进记录、促销记录等。**重大更新**：适配单合作伙伴架构，优化数据模型关系。Prisma Schema 定义了表结构与索引，迁移脚本确保数据库演进。
...
- **重大更新**：订单服务实现 OnApplicationBootstrap 接口，支持应用启动时的自动修复
...
- **新增** - 原子操作性能：updateMany 操作比多次 update 更高效
- **新增** - 异步处理：推广奖励发放采用异步方式，不阻塞主流程
- **新增** - 幂等优化：hasCharged 标记减少重复计算开销
...
## 结论
平台会员管理通过清晰的分层设计与领域驱动实现了从购买到权益、从合作伙伴到收益分配的完整闭环。**重大更新**：系统已成功重构为单合作伙伴架构，大幅简化了业务流程。新增的应用启动自动修复机制确保了数据一致性，而基于原子更新和乐观锁模式的并发安全保障提升了系统的健壮性。模块化与契约化（DTO）提升了可维护性与可测试性；事务与缓存策略保障了数据一致性与性能。建议在集成时严格遵循 DTO 约定，并关注有效期与抵扣规则的边界场景，以及并发安全的最佳实践。
...
- 合作伙伴审核
  - 申请与历史记录分离，兼容旧版审核字段，**新增** - 单合作伙伴逻辑
- 收益分配
  - 促销奖励按规则结算，支持统计与对账，**新增** - 幂等保护与自动修复
- **新增** - 并发安全最佳实践
  - 使用原子更新操作替代多次数据库查询
  - 实现幂等保护防止重复操作
  - 合理设计事务边界确保数据一致性
