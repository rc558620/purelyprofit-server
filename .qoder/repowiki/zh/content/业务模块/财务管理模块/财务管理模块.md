...
## 更新摘要
**变更内容**
- 日期范围计算重写：新增上海时区支持的日历对齐周期计算函数
- 现金流过滤函数增强：全面支持上海时区，确保与SQL查询一致的时间桶对齐
- 对账服务查询能力提升：增加状态筛选、类型筛选和全文搜索功能
- 响应DTO详细化：完善对账单响应数据结构，包含完整的字段验证和API文档注解
...
**更新** 本次更新重点改进了时间处理逻辑和对账功能，增强了系统在不同时区环境下的准确性和一致性。
...
- [finance.types.ts:1-289](file://src/purely-profit/finance/finance.types.ts#L1-L289)
...
**更新** 新增了上海时区支持的工具函数，确保跨时区环境下时间计算的准确性。
...
**更新** 增强了时区处理能力，确保在全球部署环境下时间计算的一致性。
...
**更新** 现金流过滤函数现已完全支持上海时区，确保时间范围计算与SQL查询保持一致。
...
**更新** 报表时间范围计算现已完全支持上海时区，确保跨时区环境下的报表数据一致性。
...
**更新** 新增的上海时区处理工具函数被广泛引用，确保整个财务模块的时间计算一致性。
...
**更新** 对账服务引入了可刷新缓存机制，显著提升了列表查询的性能表现。
...
**更新** 本次改进显著增强了系统的时区兼容性和对账功能，特别是上海时区的完整支持和响应DTO的详细化，为国际化部署提供了更好的技术基础。
...
**更新** 新增了上海时区处理的开发规范，确保所有时间相关功能的时区一致性。
