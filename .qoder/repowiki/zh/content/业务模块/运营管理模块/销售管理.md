...
## 更新摘要
**变更内容**   
- **团购字段支持增强**：销售记录域模型新增了完整的团购字段支持，通过buildGrouponResponseFields函数提取和转换团购元数据
- **API响应优化**：提供干净的API响应，包含可选字段仅在相关数据存在时显示
- **金额格式转换**：支持从数据库格式（分）到输出格式（元）的金额转换
- **数据完整性保障**：确保团购相关字段的完整性和一致性
...
**重要更新** 系统已实施重大架构改进，引入权威金额覆盖机制，强化了 SalesRecordAmountsDomain 的金额聚合能力，移除了过期的班次操作员解析逻辑，并改进了空间会话结账时的抵扣项处理逻辑。**新增完整的团购字段支持，提供更完善的平台结算管理能力**。
...
**重大更新** 引入了权威金额覆盖机制，支持空间结账等复杂场景下的金额覆盖；增强了 SalesRecordAmountsDomain 的金额聚合能力；移除了过期的班次操作员解析逻辑；**新增了完整的团购字段支持和响应映射功能**。
...
## 架构总览
销售管理采用"控制器-服务-领域流程-外部模块协同"的分层架构。写入路径通过事务保证一致性，同时联动库存与财务模块，确保销售发生后的库存扣减与现金流入账同步完成。**新的架构引入了权威金额覆盖机制，支持复杂场景下的金额覆盖需求**。**新增了空间结算服务的改进，正确处理抵扣项逻辑**。**新增了完整的团购字段支持，提供平台结算管理能力**。
...
**更新** 写入服务现在依赖金额聚合域服务，确保所有利润计算的一致性。**新增统计服务依赖 Redis 缓存和权限校验模块**。**新增销售记录域服务依赖 Money 工具和商业工具**。**新增空间结算服务依赖商业工具和锁机制**。
...
## 结论
销售管理模块通过清晰的分层设计与事务化流程，实现了从订单创建到库存扣减与财务入账的闭环。**最新的架构改进引入了权威金额覆盖机制，进一步强化了后端作为唯一权威来源的地位，彻底杜绝了前端篡改利润值的风险**。**SalesRecordAmountsDomain 的增强提供了更强大的金额聚合能力，支持复杂的抵扣项处理**。**空间会话结账的改进确保了预付款和续费抵扣的正确处理**。**新增的销售统计功能提供了实时的汇总统计能力，包括总收入、总利润、订单数量和平均订单价值等关键指标，支持多种时间周期的统计查询**。**新增的完整团购字段支持提供了完善的平台结算管理能力，支持从数据库格式到输出格式的自动转换**。**销售记录域服务的增强确保了团购相关数据的完整性和一致性**。业务分析与仪表盘提供了多维聚合与趋势洞察，财务与库存模块则保障了销售事实与财务、库存数据的一致性。建议在高并发场景下优化索引与事务粒度，并完善异常监控与重试策略。

**重要提示** 由于权威金额覆盖机制的引入，空间结账等复杂场景需要正确传递 totalRevenueOverride 和 totalProfitOverride 参数。**新增的统计功能需要前端正确传递时间范围参数，以获得准确的统计结果**。**新增的团购字段支持需要前端正确处理可选字段，确保只在相关数据存在时显示**。**移除了过期的班次操作员解析逻辑，简化了操作员归属逻辑**。
...
- 创建销售记录
  ...
  - 请求体：参见 DTO 定义
  - 响应：订单号与编号
  - **重要更新**：支持 totalRevenueOverride 和 totalProfitOverride 参数用于金额覆盖，以及完整的团购字段支持
...
- **新增** 销售记录响应 DTO：包含完整的团购字段支持
  - 参考路径：[sales-record.dto.ts:406-501](file://src/purely-profit/operations/sales-record/dto/sales-record.dto.ts#L406-L501)
