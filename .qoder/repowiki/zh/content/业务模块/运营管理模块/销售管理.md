...
## 更新摘要
**变更内容**   
- 重大更新：利润计算逻辑重构，后端成为唯一权威来源，不再接受前端传入的利润值
- 新增财务数据完整性策略，所有业务金额必须由后端计算以确保数据一致性
- 强化销售记录金额聚合域，统一利润推导与汇总计算
- 完善商品利润计算函数，确保利润计算的准确性与安全性
...
**重要更新** 系统已实施重大利润计算逻辑变更，后端现在作为唯一的利润计算权威来源，彻底杜绝前端传入利润值可能带来的数据篡改风险，确保财务数据的完整性和一致性。
...
**重大更新** 新增金额聚合域服务和强化的利润计算逻辑，后端现在完全控制所有利润计算，前端传入的利润字段仅用于兼容性展示，不参与实际计算。
...
## 架构总览
销售管理采用"控制器-服务-领域流程-外部模块协同"的分层架构。写入路径通过事务保证一致性，同时联动库存与财务模块，确保销售发生后的库存扣减与现金流入账同步完成。**新的架构强化了后端作为唯一权威来源的地位，所有利润计算都在服务端完成**。
...
W->>PREP : "prepareItems() - 标准化明细"
PREP->>AMOUNTS : "aggregateFromPreparedItems()"
AMOUNTS->>AMOUNTS : "后端强制计算利润"
AMOUNTS-->>W : "返回权威金额快照"
W->>FLOW : "事务内创建订单"
FLOW->>INV : "扣减库存"
FLOW->>FIN : "生成现金流"
...
**更新** 写入服务现在依赖金额聚合域服务，确保所有利润计算的一致性。
...
## 结论
销售管理模块通过清晰的分层设计与事务化流程，实现了从订单创建到库存扣减与财务入账的闭环。**最新的利润计算逻辑重构进一步强化了后端作为唯一权威来源的地位，彻底杜绝了前端篡改利润值的风险**。业务分析与仪表盘提供了多维聚合与趋势洞察，财务与库存模块则保障了销售事实与财务、库存数据的一致性。建议在高并发场景下优化索引与事务粒度，并完善异常监控与重试策略。

**重要提示** 由于利润计算逻辑的重大变更，前端应用需要进行相应的适配，确保不再尝试传入或修改利润值，而是完全信任后端计算的利润数据。
...
- 创建销售记录
  ...
  - 示例路径：[sales-record.controller.ts:1-200](file://src/purely-profit/operations/sales-record/sales-record.controller.ts#L1-L200)，[sales-record.dto.ts:1-441](file://src/purely-profit/operations/sales-record/dto/sales-record.dto.ts#L1-L441)
- 删除销售记录
  - 方法与路径：DELETE /sales-records/:id
  - 行为：回滚库存与财务现金流
  - 示例路径：[sales-record-write.service.ts:103-137](file://src/purely-profit/operations/sales-record/sales-record-write.service.ts#L103-L137)
- 获取业务分析
