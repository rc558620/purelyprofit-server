...
## 更新摘要
**所做更改**
- **4小时宽限期逻辑重大增强**：新增班次结束后4小时内仍可引用该班次的交班确认机制，提升操作灵活性
- **额外物品快照系统完善**：实现item_name_snapshot字段和外键RESTRICT约束，防止定义删除后历史数据丢失
- **收入计算逻辑优化**：改进退款和续费溢出处理，确保财务数据准确性
- **错误处理机制增强**：提升并发冲突处理和幂等性保证，增强系统稳定性
- **团购券支付映射增强**：支持平台特定标签显示和支付桶分类
...
**重大增强更新** 本次更新重点反映了交接班模块的重大增强：新增4小时宽限期逻辑允许引用刚结束的班次进行交班确认、额外物品快照系统防止定义删除后数据丢失、收入计算改进确保退款正确处理、错误处理增强提升系统稳定性。这些改进显著提升了系统的健壮性和用户体验。
...
- **服务层**：
  ...
  - 交接班确认服务：负责交接班确认流程的校验与落库，**已增强** 支持4小时宽限期逻辑。
  - 交接班记录服务：负责记录的增删改查、详情与汇总。
  - 交接班额外事项服务：负责交接班补充事项的维护，**已增强** 支持快照系统和外键保护。
  - 交接班记录查询与收入统计：负责历史记录检索与收入汇总，**已优化** 退款计算逻辑。
- **页面处理模块（新增）**：
  - 订单处理模块：专门处理订单项目、退款和客人应付项目的构建逻辑，**已增强** 支持退款和客人应付计算。
  - 支付处理模块：专注于支付方式统计、占比计算和收入汇总，**已增强** 支持团购券支付桶分类和退款金额计算。
...
Confirm --> Settle["交接班结算<br/>生成财务流水/更新账户余额<br/>优化退款计算"]
...
- **重大增强**：新增4小时宽限期逻辑，允许在班次结束后4小时内仍可引用该班次进行交班确认。

#### 4小时宽限期逻辑详解
**新增** 详细介绍4小时宽限期逻辑的实现，这是本次更新的核心特性之一：

##### 宽限期机制设计
- **核心目的**：允许在班次结束后4小时内仍可引用该班次进行交班确认
- **时间窗口**：`HANDOVER_SHIFT_GRACE_HOURS = 4` 小时
- **适用场景**：解决交班时刻晚于班次结束时间的业务需求

##### 技术实现细节
```mermaid
flowchart TD
A["班次结束时间"] --> B["计算宽限结束时间<br/>endAt + 4小时"]
B --> C{"当前交班时间是否在宽限期内？"}
C --> |是| D["视为活跃班次<br/>允许交班确认"]
C --> |否| E["视为非活跃班次<br/>按正常逻辑处理"]
```
...
##### 班次匹配算法增强
- **宽限窗口计算**：`graceEndAt = shiftRange.endAt + HANDOVER_SHIFT_GRACE_HOURS`
- **时间范围判断**：`startAt <= handoverAt <= graceEndAt`
- **优先级排序**：先尝试精确类型匹配，再返回第一个活跃班次
...
C->>S : 调用确认流程
S->>CF : 校验数据与金额
S->>CS : 处理班次切换/状态变更<br/>应用4小时宽限期逻辑
S->>DB : 写入交接班记录与快照
DB-->>S : 确认写入成功
S-->>C : 返回确认结果
C-->>U : 显示确认完成
...
- **重大增强**：优化空间会话营收计算逻辑，确保退款和续费溢出的正确处理。

#### 退款计算逻辑优化
**更新** 详细介绍退款计算逻辑的改进：

##### 综合计算规则
- **退款公式**：`refundAmount = prepaidAmount + renewTotal - consumption`
...
- 与交接班记录关联，形成完整的交接档案。
- **重大增强**：新增额外物品快照系统，防止定义删除后历史数据丢失。

#### 额外物品快照系统详解
**新增** 详细介绍额外物品快照系统的实现，这是本次更新的核心特性之一：
...
##### 映射逻辑增强
- **优先读取快照**：`mapRecordAdditionalItems` 函数优先使用 `itemNameSnapshot` 字段
- **回退机制**：当快照为空时回退到 `item.name`，确保兼容性
- **空值保护**：最终回退到"未知附加项"，避免 NPE 异常
...
#### 并发冲突处理
- **TOCTOU 竞争条件**：应用层校验通过但数据库唯一约束冲突时的处理
...
**重大增强总结** 本次更新实现了交接班模块的重大增强：4小时宽限期逻辑允许在班次结束后4小时内仍可引用该班次进行交班确认，显著提升操作灵活性；额外物品快照系统有效防止了定义删除后的数据丢失；收入计算改进确保了财务数据的准确性；错误处理增强提高了系统的稳定性；团购券支付映射增强提供了更准确的支付方式分类和显示。这些改进显著提升了用户体验和业务可靠性。建议在后续版本中继续完善数据完整性保护和性能优化，进一步提升系统的健壮性。
...
- **场景五：维护交接班额外事项**
  ...
  - 关键点：新增/编辑/删除交接事项，与交接班记录关联，**已增强** 支持快照系统和外键保护。
...
- **快照系统集成**：**新增** 额外物品快照系统与主数据解耦，提升数据完整性和系统稳定性。
- **宽限期集成**：**新增** 4小时宽限期逻辑与班次选择系统集成，提升操作灵活性。
...
#### 额外物品快照迁移
- **字段添加**：为 `store_handover_additional_values` 表添加 `item_name_snapshot` 字段
- **数据回填**：从 `store_handover_additional_items` 表回填现有记录的快照数据
- **约束变更**：将外键约束从 CASCADE 改为 RESTRICT，保护历史数据完整性
- **非空约束**：回填完成后设置 NOT NULL 约束，确保数据完整性

#### 兼容性保证
- **向后兼容**：现有 API 接口保持不变，不影响前端调用
- **数据回退**：当快照字段为空时，自动回退到关联查询获取名称
- **渐进式迁移**：支持新旧数据格式的混合处理
- **错误恢复**：迁移失败时可安全回滚，不影响系统正常运行
