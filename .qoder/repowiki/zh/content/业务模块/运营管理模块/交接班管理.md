...
- [scan-ordering-refund.service.ts](file://src/purely-club/scan-ordering/scan-ordering-refund.service.ts)
- [scan-ordering-order-refund-balance.service.ts](file://src/purely-profit/operations/scan-ordering/scan-ordering-order-refund-balance.service.ts)
- [scan-ordering-order-refund.service.ts](file://src/purely-profit/operations/scan-ordering/scan-ordering-order-refund.service.ts)
...
## 更新摘要
**所做更改**
- **平台结算支持增强**：新增平台结算支付方式的显示配置，支持"平台结算"支付方式的完整标识和颜色配置
- **手工录入订单合并展示优化**：改进了手工录入订单在交接报告中的合并展示逻辑，提升数据可读性和准确性
- **支付方式标签智能识别**：增强了手工补录单来源渠道的中文标签映射，支持美团团购、抖音团购等具体平台显示
- **退款处理逻辑完善**：完善了扫码点餐退款与手工录入订单退款的统一处理机制
...
**重大增强更新** 本次更新重点反映了交接班模块的重大改进：通过增强平台结算支持和优化手工录入订单的合并展示逻辑，显著提高了交接班报告的准确性和用户体验。新增的平台结算支付方式配置、手工录入订单的智能合并展示、以及支付方式标签的精准识别，为交接班管理提供了更加完善和友好的业务支持。
...
subgraph "平台结算与手工录入优化"
PSM["PAYMENT_METHOD_CONFIG<br/>平台结算配置"]
MES["resolveManualEntrySourceLabel<br/>手工录入来源标签"]
MMO["mergeDisplayedOrderItems<br/>手工录入订单合并"]
end
subgraph "扫码点餐退款集成"
SOR["SaleOrderRefund 表"]
SORS["scan-ordering-refund.service.ts"]
SOBS["scan-ordering-order-refund-balance.service.ts"]
end
subgraph "DTO层"
DTO["dto/*.ts"]
end
...
- **平台结算与手工录入优化**：
  - **平台结算支付方式配置**：新增 `platform` 支付方式类型，支持完整的标识和颜色配置（蓝色 #0ea5e9）。
  - **手工录入来源标签映射**：支持美团外卖、饿了么、美团团购、抖音团购等具体平台来源的中文标签显示。
  - **手工录入订单合并展示**：同一订单的多个商品行合并为1行展示，提升交接报告的可读性。
- **扫码点餐退款集成**：
  - SaleOrderRefund 表：存储扫码点餐退款记录，包含退款金额、支付方式、退款时间等信息。
  - 扫码点餐退款服务：处理退款任务创建、状态管理和库存恢复。
  - 退款余额服务：处理退款相关的余额调整和财务流水。
...
Page --> ScanRefunds["扫码点餐退款处理"]
ScanRefunds --> SaleOrderRefund["SaleOrderRefund 表"]
ScanRefunds --> RefundService["scan-ordering-refund.service.ts"]
ScanRefunds --> BalanceService["scan-ordering-order-refund-balance.service.ts"]
...
Svc --> Items["handover-order-item.mapper.ts"]
Svc --> AdditionalItems["handover-additional-items.service.ts"]
...
Svc --> PlatformConfig["平台结算配置<br/>PAYMENT_METHOD_CONFIG"]
Svc --> ManualEntry["手工录入处理<br/>resolveManualEntrySourceLabel"]
PlatformConfig --> PaymentMethod["platform: '平台结算'"]
ManualEntry --> SourceLabels["美团外卖/饿了么/美团团购等"]
```
...
### 平台结算支付方式配置增强
**新增** 详细介绍平台结算支付方式的配置增强功能：

#### 支付方式配置扩展
- **新增 platform 支付方式**：在 PAYMENT_METHOD_CONFIG 中添加了 `platform` 类型的支付方式配置
- **视觉标识**：使用蓝色 (#0ea5e9) 作为平台结算的专属颜色，区别于其他支付方式
- **标签显示**：统一显示为"平台结算"，便于用户在交接班报告中快速识别

#### 技术实现细节
```mermaid
flowchart TD
A["SalesPaymentMethod.platform"] --> B["PAYMENT_METHOD_CONFIG"]
B --> C["label: '平台结算'"]
B --> D["color: '#0ea5e9'"]
C --> E["交接班报告展示"]
D --> F["支付方式统计"]
...
#### 财务结算集成
- **平台到账分类**：在财务系统中，平台结算归类为"平台到账"，属于收入类别
- **手动创建支持**：允许手动创建平台到账的财务流水，便于特殊情况处理
- **概览聚合**：平台到账金额聚合到"附加收入"概览中，便于财务报表分析
...
### 手工录入订单合并展示优化
**新增** 详细介绍手工录入订单在交接报告中的合并展示逻辑优化：

#### 合并展示算法
- **订单分组**：同一订单的多个商品行按照 order.id 进行分组
- **首行优先**：取第一条商品行（id最小）作为基础信息，包括商品名、支付方式等
- **金额汇总**：整单金额为所有商品行金额的总和，数量固定为1
- **退款对齐**：与退款行组成「下单 + 退款」两行展示，与非录入单子退款展示口径一致

#### 技术实现优势
```mermaid
flowchart TD
A["手工录入订单"] --> B["按order.id分组"]
B --> C["取第一条商品行"]
C --> D["mapOrderItem处理"]
D --> E["金额汇总计算"]
E --> F["合并为单一展示行"]
...
#### 显示效果改进
- **简洁明了**：避免同一订单的多行重复显示，减少视觉干扰
- **信息完整**：保留订单的完整信息，包括商品名、支付方式、操作员等
- **一致性**：与非录入订单的展示逻辑保持一致，提升用户体验

**章节来源**
- [handover-page-order-items.ts](file://src/purely-profit/operations/handover/handover-page-order-items.ts)

### 支付方式标签智能识别
**新增** 详细介绍支付方式标签的智能识别功能：

#### 手工录入来源渠道映射
- **多渠道支持**：支持美团外卖、饿了么、美团团购、抖音团购、大众点评等具体平台来源
- **中文标签**：将英文渠道代码转换为友好的中文标签，如 "meituan" → "美团外卖"
- **大小写兼容**：支持不同大小写的渠道代码，提升兼容性

#### 标签识别优先级
```mermaid
flowchart TD
A["订单支付方式"] --> B{"是否为手工录入单？"}
B --> |是| C["检查sourceChannel"]
B --> |否| D["使用默认支付方式"]
C --> E{"有匹配的渠道标签？"}
E --> |是| F["显示具体平台标签"]
E --> |否| G["回退到默认支付方式"]
...
#### 应用场景
- **美团团购**：当 sourceChannel 为 "meituanVoucher" 时显示"美团团购"
- **抖音团购**：当 sourceChannel 为 "douyin" 时显示"抖音团购"  
- **外卖平台**：当 sourceChannel 为 "meituan" 或 "eleme" 时显示对应的平台名称
- **其他平台**：对于未知渠道，回退到通用的"其他平台"标签
...
### 扫码点餐退款集成详解
**新增** 详细介绍扫码点餐退款集成的核心功能：

#### SaleOrderRefund 表结构
- **退款记录存储**：包含退款金额、利润、支付方式、退款时间等关键字段
- **关联关系**：与销售订单、门店、财务流水建立关联关系
- **索引优化**：针对 storeId 和 refundedAt 字段建立索引，提升查询性能

#### 扫码点餐退款服务
- **退款任务创建**：支持异常支付和商家拒单两种触发类型
- **状态管理**：管理退款任务的待处理、退款中、手动待处理等状态
- **实时通知**：通过 WebSocket 推送退款状态变更通知

#### 退款余额调整
- **余额恢复**：处理退款相关的余额调整和资金回退
- **库存恢复**：自动恢复退款商品的库存数量
- **财务流水**：生成对应的财务流水记录，确保账目平衡

#### 交接班页面集成
- **退款数据加载**：在交接班页面中加载当前班次内的扫码点餐退款记录
- **退款项展示**：将扫码点餐退款以负数形式展示，与下单行并存保证账目平衡
- **退款统计**：将扫码点餐退款与空间会话退款双重累加，确保退款金额的准确性
...
- **扫码点餐退款依赖**：交接班系统与扫码点餐退款服务深度集成，确保退款数据的完整性和准确性。
- **平台结算依赖**：新增的平台结算支付方式配置与现有的支付方式系统无缝集成。
- **手工录入依赖**：手工录入订单的合并展示逻辑与订单处理系统紧密协作。
...
S --> DB["Prisma 模式"]
S --> SRS["scan-ordering-refund.service.ts"]
S --> SOBS["scan-ordering-order-refund-balance.service.ts"]
S --> PlatformConfig["平台结算配置"]
S --> ManualEntry["手工录入处理"]
PlatformConfig --> PaymentMethod["platform: '平台结算'"]
ManualEntry --> SourceLabels["美团外卖/饿了么/美团团购等"]
```
...
- **扫码点餐退款性能**：SaleOrderRefund 查询使用索引优化，退款任务处理采用异步队列，避免阻塞主流程。
- **平台结算配置性能**：支付方式配置采用静态常量，无需运行时计算，性能开销极低。
- **手工录入合并性能**：订单合并逻辑使用Map结构进行分组，时间复杂度为O(n)，性能表现良好。
...
- **扫码点餐退款异常**：检查 SaleOrderRefund 表数据完整性，确认退款任务状态和库存恢复逻辑。
- **退款展示异常**：检查 mapScanOrderingRefundOrderItem 函数的退款项映射逻辑，确认负数金额和支付方式标签的正确性。
- **平台结算显示异常**：检查 PAYMENT_METHOD_CONFIG 中 platform 类型的配置是否正确，确认支付方式标签显示逻辑。
- **手工录入合并异常**：检查 mergeDisplayedOrderItems 函数的订单分组逻辑，确认手工录入订单的合并展示是否正确。
- **来源渠道标签异常**：检查 resolveManualEntrySourceLabel 函数的渠道映射逻辑，确认手工录入单的来源渠道标签显示。
...
**重大增强总结** 本次更新实现了交接班模块的重大改进：通过增强平台结算支持和优化手工录入订单的合并展示逻辑，显著提高了交接班报告的准确性和用户体验。新增的平台结算支付方式配置、手工录入订单的智能合并展示、以及支付方式标签的精准识别，为交接班管理提供了更加完善和友好的业务支持。建议在后续版本中继续完善数据完整性保护和性能优化，进一步提升系统的健壮性。
...
- **场景十七：处理扫码点餐退款**
  - 路径参考：[scan-ordering-refund.service.ts](file://src/purely-club/scan-ordering/scan-ordering-refund.service.ts)
  - 关键点：创建退款任务、管理退款状态、恢复商品库存和生成财务流水。

- **场景十八：交接班页面展示扫码点餐退款**
  - 路径参考：[handover-page.service.ts](file://src/purely-profit/operations/handover/handover-page.service.ts)
  - 关键点：加载当前班次内的扫码点餐退款记录，以负数形式展示保证账目平衡。

- **场景十九：处理平台结算支付方式**
  - 路径参考：[handover.constants.ts](file://src/purely-profit/operations/handover/handover.constants.ts)
  - 关键点：使用新增的 platform 支付方式类型，显示"平台结算"标签和蓝色标识。

- **场景二十：手工录入订单合并展示**
  - 路径参考：[handover-page-order-items.ts](file://src/purely-profit/operations/handover/handover-page-order-items.ts)
  - 关键点：同一订单的多个商品行合并为1行展示，提升交接报告的可读性。

- **场景二十一：手工录入来源渠道标签**
  - 路径参考：[handover-order-item.mapper.ts](file://src/purely-profit/operations/handover/handover-order-item.mapper.ts)
  - 关键点：将英文渠道代码转换为中文标签，如"meituanVoucher"显示为"美团团购"。
...
- **扫码点餐退款集成**：交接班系统与扫码点餐退款服务深度集成，确保退款数据的完整性和准确性。
- **平台结算集成**：新增的平台结算支付方式与现有支付系统无缝集成，支持完整的财务流转。
- **手工录入集成**：手工录入订单的合并展示逻辑与订单处理系统紧密协作，提升用户体验。
...
### 扫码点餐退款技术细节
**新增** 详细介绍扫码点餐退款集成的技术实现：

#### SaleOrderRefund 表设计
- **核心字段**：id、saleOrderId、storeId、amount、profit、paymentMethod、refundedAt
- **关联关系**：与销售订单、门店、财务流水建立一对一或多对一关系
- **索引优化**：针对 storeId 和 refundedAt 字段建立复合索引，提升查询性能

#### 退款任务管理
- **任务创建**：支持异常支付和商家拒单两种触发类型
- **状态流转**：pending → refunding → succeeded/failed
- **重试机制**：失败任务支持自动重试和手动重试

#### 余额和库存恢复
- **余额调整**：根据退款金额调整用户余额和门店收益
- **库存恢复**：自动恢复退款商品的库存数量
- **财务流水**：生成对应的财务流水记录，确保账目平衡

#### 交接班页面集成
- **数据加载**：在交接班页面中加载当前班次内的扫码点餐退款记录
- **展示逻辑**：将扫码点餐退款以负数形式展示，与下单行并存保证账目平衡
- **统计汇总**：将扫码点餐退款与空间会话退款双重累加，确保退款金额的准确性
...
### 平台结算支付方式技术细节
**新增** 详细介绍平台结算支付方式的技术实现：

#### 支付方式配置
- **枚举扩展**：在 SalesPaymentMethod 中新增 platform 类型
- **配置定义**：在 PAYMENT_METHOD_CONFIG 中添加 platform 的配置项
- **视觉标识**：使用蓝色 (#0ea5e9) 作为平台结算的专属颜色
- **标签显示**：统一显示为"平台结算"，便于用户识别

#### 财务结算集成
- **分类规则**：在财务系统中，平台结算归类为"平台到账"
- **方向定义**：平台到账属于收入类别，direction 为 'income'
- **手动创建**：允许手动创建平台到账的财务流水
- **概览聚合**：平台到账金额聚合到"附加收入"概览中

#### 显示逻辑优化
- **优先级处理**：平台结算支付方式在交接班报告中正确显示
- **颜色标识**：使用配置的蓝色标识，与其他支付方式区分
- **统计归类**：在支付方式统计中正确归类到平台结算类别
...
### 手工录入订单合并技术细节
**新增** 详细介绍手工录入订单合并展示的技术实现：

#### 订单分组算法
- **分组依据**：按照 order.id 对订单进行分组
- **首行选择**：取 id 最小的商品行作为基础信息
- **金额汇总**：计算整单金额为所有商品行金额的总和
- **数量处理**：合并后的数量固定为1，表示一笔录入单

#### 展示逻辑优化
- **商品信息**：使用第一条商品行的商品名、规格等信息
- **支付方式**：继承第一条商品行的支付方式信息
- **操作员信息**：继承第一条商品行的操作员信息
- **时间戳**：使用第一条商品行的时间戳

#### 退款对齐处理
- **退款匹配**：退款行与合并后的下单行形成对应关系
- **展示一致性**：与未合并的订单退款展示逻辑保持一致
- **账目平衡**：确保「下单 + 退款」两行的金额平衡

**章节来源**
- [handover-page-order-items.ts](file://src/purely-profit/operations/handover/handover-page-order-items.ts)

### 手工录入来源渠道标签技术细节
**新增** 详细介绍手工录入来源渠道标签的技术实现：

#### 渠道映射配置
- **映射表**：MANUAL_ENTRY_SOURCE_CHANNEL_LABEL 定义了渠道代码到中文标签的映射
- **支持渠道**：包括 meituan(美团外卖)、eleme(饿了么)、meituanVoucher(美团团购)、douyin(抖音团购)等
- **大小写兼容**：支持不同大小写的渠道代码，提升兼容性
- **回退机制**：对于未知渠道，返回空字符串，使用默认支付方式

#### 标签解析逻辑
- **输入处理**：接收 sourceChannel 字段作为输入
- **匹配查找**：在映射表中查找对应的中文标签
- **大小写处理**：支持 toLowerCase() 后的值匹配
- **空值处理**：对于空值或 null 值，返回空字符串

#### 显示优先级
- **手工录入优先**：对于手工录入订单，优先显示具体的来源渠道标签
- **回退机制**：如果没有匹配的渠道标签，回退到默认的支付方式标签
- **颜色继承**：标签的颜色继承自原支付方式配置的颜色
