...
## 更新摘要
**所做更改**
- **团购券支付映射增强**：为团购券交易创建单独的支付桶，确保准确的支付方式分类和报告
- **支付处理逻辑改进**：在开台项（预付款/台位费）场景下，当顾客使用团购券时归入"团购"桶而非门店结算方式
- **显示覆盖机制**：实现了基于顾客实际支付方式的显示覆盖，确保前端正确展示"团购"标识
- **测试用例完善**：增加了针对团购券支付处理的完整测试用例，验证支付桶分类的正确性
...
**重大支付处理增强** 本次更新重点反映了团购券支付映射的改进：系统现在能够为团购券交易创建单独的支付桶，确保在交接班统计中准确区分团购券支付与其他支付方式。这一改进特别适用于开台项（预付款/台位费）场景，当顾客使用团购券时，系统将自动将其归入"团购"桶，而不是按照门店侧的结算方式进行归类。
...
### 团购券支付映射增强详解
**新增** 详细介绍团购券支付映射的增强功能，这是本次更新的核心改进：
...
#### 显示覆盖机制
- **显示标签**：团购券交易在前端显示为"团购"，颜色为 `#b45309`
...
B --> |是| C{"顾客支付方式为团购券？"}
B --> |否| D["按门店结算方式归类"]
C --> |是| E["归入团购桶<br/>label: '团购', color: '#b45309'"]
C --> |否| D
D --> F["使用标准支付方式配置"]
E --> G["添加到支付统计"]
F --> G
...
- **结算渠道字段**：`SpaceSession.prepaidSettlementChannel` 记录结算渠道（美团团购、抖音团购等）
...
**重大支付处理增强总结** 本次团购券支付映射增强显著提升了交接班统计的准确性和业务灵活性。通过为团购券交易创建单独的支付桶，我们实现了更精确的支付方式分类和报告，特别是在开台项场景中能够正确区分顾客实际使用的团购券与门店侧的结算方式。这一改进不仅提高了数据统计的准确性，还为后续的财务分析和业务洞察提供了更可靠的数据基础。建议在后续版本中继续完善支付方式的扩展机制，支持更多第三方平台的团购券处理。
...
- **场景十：处理团购券开台支付**
  - 路径参考：[handover-page-payment.ts](file://src/purely-profit/operations/handover/handover-page-payment.ts)
  - 关键点：当开台项顾客使用团购券时，自动归入"团购"支付桶，显示正确的标签和颜色。
