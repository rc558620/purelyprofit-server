...
- [src/purely-profit/auth/auth-login-fail-guard.service.ts](file://src/purely-profit/auth/auth-login-fail-guard.service.ts)
...
## 更新摘要
**变更内容**
- 新增基于 Redis 的登录失败防护服务，支持可配置阈值和自动临时锁定机制
- 增强跨租户冲突检测机制
- 改进会话管理，实现密码变更后立即失效的 Token 版本递增机制
- 完善安全审计日志记录，包括账号锁定事件
...
**更新** 本次更新重点介绍了新增的登录失败防护机制、增强的跨租户冲突检测和改进的会话管理功能。
...
- **新增** 登录失败防护：基于 Redis 的原子计数和自动锁定机制
...
ALFGS["AuthLoginFailGuardService<br/>登录失败防护"]
end
subgraph "外部依赖"
RS["RedisService"]
CFG["ConfigService"]
...
- [src/purely-profit/auth/auth-password.service.ts](file://src/purely-profit/auth/auth-password.service.ts)
- [src/purely-profit/auth/auth-login-fail-guard.service.ts:19-24](file://src/purely-profit/auth/auth-login-fail-guard.service.ts#L19-L24)
- [src/redis/redis.service.ts](file://src/redis/redis.service.ts)
...
- **新增** 登录失败防护：基于 Redis INCR 原子递增记录登录失败次数，达到阈值后自动临时锁定账号。
...
## 架构总览
下图展示认证与会话的关键交互：客户端发起登录/注册请求，服务端生成验证码并发送短信，完成校验后签发 JWT；后续请求由守卫与策略校验 Token 并结合会话版本进行有效性检查。**新增** 登录失败防护机制在每次登录尝试时进行检查和计数。

```mermaid
sequenceDiagram
participant C as "客户端"
participant S as "AuthService"
participant LFGS as "AuthLoginFailGuardService"
...
- [src/purely-profit/auth/auth-login-fail-guard.service.ts:29-42](file://src/purely-profit/auth/auth-login-fail-guard.service.ts#L29-L42)
- [src/redis/redis.service.ts](file://src/redis/redis.service.ts)
...
- **改进** 密码变更后立即失效：通过 Token 版本递增机制，确保密码修改后所有现有会话立即失效。
...
Note over SS,J : "密码变更后版本递增，旧Token立即失效"
```

**图表来源**
- [src/purely-profit/auth/auth-session.service.ts:16-46](file://src/purely-profit/auth/auth-session.service.ts#L16-L46)
- [src/purely-profit/auth/strategies/jwt.strategy.spec.ts:39-63](file://src/purely-profit/auth/strategies/jwt.strategy.spec.ts#L39-63)
- [src/redis/redis.service.ts](file://src/redis/redis.service.ts)
...
### 登录失败防护机制
**新增** 基于 Redis 的登录失败防护服务，提供以下核心功能：

- **原子计数**：使用 Redis INCR 命令原子递增登录失败次数，确保并发安全
- **自动锁定**：当失败次数达到阈值（默认5次）时，自动临时锁定账号15分钟
- **智能提醒**：在接近锁定时（剩余2次机会）向用户发出警告提示
- **审计日志**：记录账号锁定事件，包含产品范围、失败次数等元数据
- **自动清理**：登录成功后自动清除失败计数

```mermaid
flowchart TD
Start(["登录尝试"]) --> CheckLocked["检查是否已锁定"]
CheckLocked --> Locked{"已锁定？"}
Locked -- 是 --> LockError["抛出锁定异常"]
Locked -- 否 --> Auth["验证用户名密码"]
Auth --> Success{"验证成功？"}
Success -- 是 --> ClearCount["清除失败计数"]
ClearCount --> SuccessResp["返回登录成功"]
Success -- 否 --> IncrCount["Redis INCR 递增失败计数"]
IncrCount --> CheckThreshold{"达到阈值？"}
CheckThreshold -- 是 --> SetLock["设置锁定时长 + 记录审计日志"]
SetLock --> LockError
CheckThreshold -- 否 --> WarningCheck{"需要警告？"}
WarningCheck -- 是 --> WarnResp["返回警告消息"]
WarningCheck -- 否 --> NormalError["返回普通错误"]
```

**图表来源**
- [src/purely-profit/auth/auth-login-fail-guard.service.ts:29-42](file://src/purely-profit/auth/auth-login-fail-guard.service.ts#L29-L42)
- [src/purely-profit/auth/auth-login-fail-guard.service.ts:48-78](file://src/purely-profit/auth/auth-login-fail-guard.service.ts#L48-L78)
- [src/purely-profit/auth/auth-login-fail-guard.service.ts:83-89](file://src/purely-profit/auth/auth-login-fail-guard.service.ts#L83-L89)

**章节来源**
- [src/purely-profit/auth/auth-login-fail-guard.service.ts:19-98](file://src/purely-profit/auth/auth-login-fail-guard.service.ts#L19-L98)
- [src/purely-profit/auth/auth.constants.ts:12-19](file://src/purely-profit/auth/auth.constants.ts#L12-L19)
- [src/purely-profit/auth/auth-authentication.service.ts:145-184](file://src/purely-profit/auth/auth-authentication.service.ts#L145-L184)
...
- **增强** 登录失败审计：账号锁定事件自动记录到审计日志，包含产品范围和失败次数信息。
...
Log --> Resp["返回响应"]
Note over Strategy,Log : "登录失败时记录锁定事件"
```
...
- **新增** 登录失败防护配置：通过常量定义控制最大失败次数、警告阈值、锁定时长等安全策略参数。
...
- **新增** 登录失败防护服务依赖 Redis 进行原子计数和审计日志服务进行事件记录。
...
AS --> ALFGS["AuthLoginFailGuardService"]
...
- **新增** 登录失败防护性能：使用 Redis INCR 原子操作，避免数据库竞争条件；锁定时长较短（15分钟），减少长期资源占用。
...
- **新增** 账号被锁定：检查 Redis 中登录失败计数键是否存在且未过期；查看审计日志中的锁定事件；等待锁定时长自动过期或联系管理员解锁。
...
- [src/purely-profit/auth/request-audit-context.decorator.ts:26-39](file://src/purely-profit/auth/request-audit-context.decorator.ts#L26-L39)
- [src/purely-profit/auth/auth-login-fail-guard.service.ts:65-75](file://src/purely-profit/auth/auth-login-fail-guard.service.ts#L65-L75)

## 结论
本项目在认证与会话层面已具备较为完善的基础设施：JWT 签发、Redis 会话版本控制、验证码与短信流程、请求审计上下文等。**新增** 的登录失败防护机制进一步增强了账户安全性，通过原子计数和自动锁定有效抵御暴力破解攻击。建议在此基础上进一步完善 CSRF/XSS 防护策略、密码强度规则与历史密码限制、SQL 注入的最小暴露面管理，并建立统一的安全审计与事件响应流程，以达到更高等级的安全保障。

## 附录
- 安全配置要点
  - CORS 来源白名单、Swagger 开关、日志级别、HTTP 超时与慢请求阈值
  - JWT 密钥与过期时间、Redis 连接与超时
  - **新增** 登录失败防护配置：最大失败次数（5次）、警告阈值（2次）、锁定时长（15分钟）
- 建议的增强措施
