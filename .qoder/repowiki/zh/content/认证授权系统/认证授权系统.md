...
## 更新摘要
**所做更改**
- 新增昵称更新接口，支持用户昵称的修改与验证
- 新增注册闭环门店创建服务，提供完整的注册后门店创建流程
- 增强验证码服务的速率限制保护，新增SMS冷却功能
- 改进密码重置功能，优化验证码发送与验证流程
- 新增Throttle装饰器，提供统一的请求频率限制
...
**更新** 新增昵称更新接口和注册闭环门店创建服务，提供更完整的用户资料管理和业务流程支持。增强的验证码服务通过SMS冷却功能实现精确的速率限制，有效防止恶意刷短信攻击，确保系统的公平使用。改进的密码重置功能提升了用户体验和安全性。
...
- **登录与 Token 生成**
  ...
  - 生成策略参考：[auth.service.ts:1-122](file://src/purely-profit/auth/auth.service.ts#L1-L122)、[auth-authentication.service.ts:1-200](file://src/purely-profit/auth/auth-authentication.service.ts#L1-L200)、[auth.constants.ts:1-28](file://src/purely-profit/auth/auth.constants.ts#L1-L28)。
- **JWT 解析与验证**
...
- **Token 生命周期与刷新**
  ...
  - 参考：[auth.constants.ts:1-28](file://src/purely-profit/auth/auth.constants.ts#L1-L28)、[configuration.ts:1-243](file://src/config/configuration.ts#L1-L243)。
...
- [jwt-auth.guard.ts:1-200](file://src/purely-profit/auth/guards/jwt-auth.guard.ts#L1-L200)
- [jwt.strategy.ts:1-200](file://src/purely-profit/auth/strategies/jwt.strategy.ts#L1-L200)
- [auth.constants.ts:1-28](file://src/purely-profit/auth/auth.constants.ts#L1-L28)
...
UPS --> AAMS["AuthAccountMembershipService"]
UPS --> ACS
end
subgraph "注册闭环服务"
...
- **Token 无效或过期**
  ...
  - 参考：[jwt.strategy.ts:1-200](file://src/purely-profit/auth/strategies/jwt.strategy.ts#L1-L200)、[auth.constants.ts:1-28](file://src/purely-profit/auth/auth.constants.ts#L1-L28)。
...
**更新** 新增的SMS冷却功能通过Redis setIfAbsent操作实现了精确的速率限制，有效防止了恶意刷短信攻击，确保了系统的公平使用。该功能支持注册、密码重置和登录操作的独立冷却周期，提升了系统的安全性和稳定性。

**更新** 新增的昵称更新接口和注册闭环门店创建服务提供了更完整的用户资料管理和业务流程支持。用户可以在登录后快速更新昵称，注册完成后可以立即创建门店，提升了用户体验和业务效率。

**更新** Throttle装饰器的引入提供了统一的请求频率限制功能，通过Redis实现分布式限流，有效防止了恶意请求和DDoS攻击，确保了系统的稳定性和可靠性。

子账户体系通过登录服务、槽位服务与阻断守卫形成闭环，既保证灵活性又强化安全边界。配合数据库与缓存的合理使用，系统可在保证安全性的同时兼顾性能与可维护性。新的模块架构为未来的功能扩展和维护提供了更好的基础。

## 附录
- **关键常量与配置**
  - 认证相关常量与 Token 参数定义：[auth.constants.ts:1-28](file://src/purely-profit/auth/auth.constants.ts#L1-L28)
