...
## 更新摘要
**所做更改**
- 重大架构重构：AuthAccountMembershipService(586行)被拆分为三个专注服务，遵循单一职责原则
- 新增AuthBanGuardService(封禁检查服务)：专门处理用户封禁状态检查和门店关联查询
- 新增AuthMembershipResolverService(会员解析与缓存服务)：负责会员信息解析、权限计算和缓存管理
- 新增AuthStaffActivationService(员工激活工作流服务)：处理邀请员工激活和席位配额管理
- 保持向后兼容的公共API表面，现有调用方无需修改代码
- 优化了缓存策略和性能，提高了代码可维护性和扩展性
...
**重大更新** 认证系统进行了重大架构重构，原有的AuthAccountMembershipService(586行)被拆分为三个专注服务：AuthBanGuardService(封禁检查)、AuthMembershipResolverService(会员解析与缓存)、AuthStaffActivationService(员工激活工作流)。这些变化遵循单一职责原则，显著提高了代码可维护性但保持了向后兼容的公共API表面。新增的昵称更新接口和注册闭环门店创建服务，提供更完整的用户资料管理和业务流程支持。增强的验证码服务通过SMS冷却功能实现精确的速率限制，有效防止恶意刷短信攻击，确保系统的公平使用。改进的密码重置功能提升了用户体验和安全性。
...
- **会员解析与缓存服务**：AuthMembershipResolverService 负责会员信息解析、权限计算、缓存管理和历史数据修复。
...
**重大架构更新** 经过重构后，认证系统采用了更加清晰的单一职责原则。AuthAccountMembershipService被拆分为三个专注服务：AuthBanGuardService专注于封禁检查，AuthMembershipResolverService专注于会员解析与缓存，AuthStaffActivationService专注于员工激活工作流。这种拆分提高了代码的可维护性和测试性，同时保持了向后兼容的公共API表面。
...
AMRS["会员解析服务<br/>AuthMembershipResolverService"] --> MainAuthSvc
...
- **模块依赖重构**
  - **重大更新** AuthAccountMembershipService被拆分为三个专注服务：
    ...
    - AuthMembershipResolverService：专注于会员解析与缓存，处理权限计算和历史数据修复
...
subgraph "重构后的认证服务层"
ABGS["AuthBanGuardService<br/>封禁检查服务"]
AMRS["AuthMembershipResolverService<br/>会员解析服务"]
...
subgraph "用户资料管理"
UPS["AuthProfileService"] --> AALS["AuthAccountLookupService"]
UPS --> ACS
end
subgraph "注册闭环服务"
...
## 结论
本认证授权系统以 JWT 为核心凭证，结合守卫与装饰器实现细粒度的权限控制，并通过主体能力服务构建可扩展的权限继承模型。经过重大架构重构后，系统采用了更加清晰的单一职责原则，将原有的AuthAccountMembershipService拆分为三个专注服务，显著提高了代码的可维护性和性能表现。

**重大更新总结** 
- **AuthBanGuardService**：专注于封禁检查，实现了高效的批量查询和缓存机制，支持门店维度的封禁判断
- **AuthMembershipResolverService**：专注于会员解析与缓存，提供了智能的权限计算和历史数据修复功能
- **AuthStaffActivationService**：专注于员工激活工作流，实现了完善的配额管理和缓存失效机制

这些变化保持了向后兼容的公共API表面，现有调用方无需修改代码即可享受重构带来的性能提升和维护性改善。新增的昵称更新接口和注册闭环门店创建服务，提供了更完整的用户资料管理和业务流程支持。增强的验证码服务通过SMS冷却功能实现了精确的速率限制，有效防止了恶意刷短信攻击。
...
子账户体系通过登录服务、槽位服务与阻断守卫形成闭环，既保证灵活性又强化安全边界。配合数据库与缓存的合理使用，系统可在保证安全性的同时兼顾性能与可维护性。新的模块化架构为未来的功能扩展和维护提供了更好的基础。
