...
**更新** 新增统一的 AuthProductAuthService，支持多产品范围认证，包括登录验证码、密码管理、短信验证等核心功能，显著增强了系统的安全性和用户体验。
...
- **purely-pulse 认证模块**：专为开发者设计的认证系统，提供受限的开发者登录入口。
...
## 架构总览
认证授权系统采用"控制器-服务-守卫-策略-装饰器"的分层设计，结合数据库与缓存完成身份认证与权限控制。JWT 作为无状态凭证贯穿整个流程；权限控制通过守卫与装饰器在路由层进行拦截与判定；子账户登录由专门的服务与守卫协同处理，确保主账户与子账户的边界清晰。新增的 AuthProductAuthService 通过统一接口支持多产品范围认证，包括验证码管理、短信发送和密码操作等核心功能。
...
### purely-pulse 认证系统
purely-pulse 是专为开发者设计的认证模块，提供受限的开发者登录入口：
...
```mermaid
flowchart TD
A["开发者尝试登录"] --> B["pulse-auth.controller 接收请求"]
B --> C["pulse-auth.service 调用 loginPulse"]
C --> D["auth.service.loginByScope(product='purely_profit', developer=true)"]
D --> E{"检查开发者白名单"}
E --> |是| F["验证凭据并生成JWT"]
E --> |否| G["拒绝访问"]
F --> H["返回JWT Token"]
...
- **组件耦合**
  ...
  - purely-club 和 purely-pulse 模块依赖主认证服务实现具体功能
...
PCtrl["pulse-auth.controller.ts"] --> PSvc["pulse-auth.service.ts"]
PSvc --> MainSvc
end
subgraph "主认证模块"
...
- **purely-pulse 开发者验证失败**
  - 现象：开发者登录被拒绝。
  - 排查：确认开发者邮箱是否在白名单中，检查配置文件设置。
  - 参考：[pulse-auth.controller.ts:1-30](file://src/purely-pulse/auth/pulse-auth.controller.ts#L1-L30)、[auth-authentication.service.ts:26-40](file://src/purely-profit/auth/auth-authentication.service.ts#L26-L40)。
...
## 结论
本认证授权系统以 JWT 为核心凭证，结合守卫与装饰器实现细粒度的权限控制，并通过主体能力服务构建可扩展的权限继承模型。新增的 AuthProductAuthService 通过统一接口支持多产品范围认证，包括验证码管理、短信发送和密码操作等核心功能，显著增强了系统的安全性和用户体验。新增的验证码服务和短信服务提供了完善的多因素认证支持，确保用户身份验证的安全性。purely-club 和 purely-pulse 认证模块通过独立的认证入口实现了产品范围的完全隔离，既保证了各业务线的独立性，又保持了统一的认证体验。子账户体系通过登录服务、槽位服务与阻断守卫形成闭环，既保证灵活性又强化安全边界。配合数据库与缓存的合理使用，系统可在保证安全性的同时兼顾性能与可维护性。
