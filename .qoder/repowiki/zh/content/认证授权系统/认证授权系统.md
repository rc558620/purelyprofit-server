...
## 更新摘要
**所做更改**
- 新增 purely-club 认证模块，包括独立的认证控制器、服务和模块
- 新增 purely-pulse 认证模块，支持开发者专用登录
- 增强认证服务，支持多产品范围的认证方法
- 更新认证流程图，反映新增的认证范围
- 新增多产品范围认证的架构说明
...
**更新** 新增 purely-club 和 purely-pulse 两个独立认证模块，支持多产品范围的认证需求，包括个人端、开发者端的差异化认证策略。
...
## 架构总览
认证授权系统采用"控制器-服务-守卫-策略-装饰器"的分层设计，结合数据库与缓存完成身份认证与权限控制。JWT 作为无状态凭证贯穿整个流程；权限控制通过守卫与装饰器在路由层进行拦截与判定；子账户登录由专门的服务与守卫协同处理，确保主账户与子账户的边界清晰。新增的 purely-club 和 purely-pulse 模块通过独立的认证入口实现产品范围隔离。
...
MainAuthSvc --> Prisma["数据库访问<br/>prisma.service.ts"]
MainAuthSvc --> Redis["缓存访问<br/>redis.service.ts"]
PermGuard --> Prisma
SubAccBlock --> Prisma
```
...
**图表来源**
- [auth-account.types.ts:1-20](file://src/purely-profit/auth/auth-account.types.ts#L1-L20)
...
- 认证范围错误
  ...
  - 参考：[auth-account.types.ts:1-20](file://src/purely-profit/auth/auth-account.types.ts#L1-L20)。
- purely-club 账户问题
...
## 结论
本认证授权系统以 JWT 为核心凭证，结合守卫与装饰器实现细粒度的权限控制，并通过主体能力服务构建可扩展的权限继承模型。新增的 purely-club 和 purely-pulse 认证模块通过独立的认证入口实现了产品范围的完全隔离，既保证了各业务线的独立性，又保持了统一的认证体验。子账户体系通过登录服务、槽位服务与阻断守卫形成闭环，既保证灵活性又强化安全边界。配合数据库与缓存的合理使用，系统可在保证安全性的同时兼顾性能与可维护性。
...
- DTO 与类型
  ...
  - 认证范围类型定义：[auth-account.types.ts:1-20](file://src/purely-profit/auth/auth-account.types.ts#L1-L20)
- 数据模型与关系
