...
- [access-control.constants.ts:1-161](file://src/purely-profit/access-control/access-control.constants.ts#L1-L161)
...
章节来源
- [access-control.constants.ts:5-54](file://src/purely-profit/access-control/access-control.constants.ts#L5-L54)
- [access-control.constants.ts:101-160](file://src/purely-profit/access-control/access-control.constants.ts#L101-L160)
...
- 调试工具与手段
  - 单元测试：利用 access-control-alignment.spec.ts 中的断言，定位控制器与方法的权限元数据装配是否正确。
...
## 结论
该权限控制模型以装饰器与守卫为核心，结合服务层的权限计算与能力快照，实现了灵活、可扩展、可审计的 RBAC 能力。通过默认角色权限与动态权限的融合、子账号差异化裁剪、以及严格的控制器级装配，既保证了易用性，也兼顾了安全性与性能。建议在生产环境中配合缓存与可观测性体系，持续优化热点路径与异常处置。
