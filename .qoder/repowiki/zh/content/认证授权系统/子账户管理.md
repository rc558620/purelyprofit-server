...
- [prisma/migrations/20260707210000_staff_login_account_field/migration.sql](file://prisma/migrations/20260707210000_staff_login_account_field/migration.sql)
</cite>

## 更新摘要
**变更内容**
- 新增 loginAccount 字段支持，实现员工认证系统重大重构
- 改进子账户登录服务的全球唯一性验证和冲突检测机制
- 增强数据完整性和事务安全性，消除 email 字段多语义编码问题
- 优化登录账号查找性能，通过独立字段直接查询替代复杂的邮箱格式解析
...
**已更新** 员工认证系统重大重构，新增 loginAccount 字段支持，改进了全球唯一性验证和冲突检测机制
...
- StoreSubAccountLoginService.ensureEmployeeHasLoginAccount：为员工建立或更新登录账号（User+Staff），**现已支持独立的 loginAccount 字段**，校验账号唯一性与密码强度。
...
Slot->>PM : "校验功能开关"
Slot->>Slot : "校验槽位索引与员工"
Slot->>Login : "为员工创建/更新登录账号支持loginAccount字段"
Login->>Login : "执行全局唯一性预检"
Login->>Login : "更新staff表的email和loginAccount字段"
Slot->>Slot : "upsert 槽位"
Slot->>Read : "重新读取汇总"
Read-->>Slot : "返回最新汇总"
Slot-->>Svc : "返回汇总"
...
- [src/purely-profit/member/platform-membership/store-sub-account-login.service.ts:37-264](file://src/purely-profit/member/platform-membership/store-sub-account-login.service.ts#L37-L264)
...
### 组件六：loginAccount 字段支持与认证系统重构

**新增** 员工认证系统重大重构，引入独立的 loginAccount 字段

- **数据库迁移**：为 staffs 表新增 login_account 字段，消除 email 字段多语义编码问题
- **登录账号验证**：StoreSubAccountLoginService 实现了增强的全局唯一性预检机制
- **认证查找优化**：AuthAccountLookupService 支持通过 loginAccount 字段直接查找用户，无需复杂的邮箱格式解析
- **向后兼容**：保留对旧格式邮箱的支持，同时支持新的独立字段查询

```mermaid
flowchart TD
A["员工登录账号设置"] --> B["验证loginAccount格式"]
B --> C["执行全局唯一性预检"]
C --> D{"是否存在冲突?"}
D --> |是| E["抛出冲突异常"]
D --> |否| F["更新staff表email和loginAccount字段"]
F --> G["创建或关联User记录"]
G --> H["完成登录账号配置"]
I["用户登录"] --> J{"尝试loginAccount直接查找"}
J --> K{"找到匹配记录?"}
K --> |是| L["返回用户信息"]
K --> |否| M["回退到邮箱格式解析"]
M --> N["返回用户信息"]
...
图表来源
- [prisma/migrations/20260707210000_staff_login_account_field/migration.sql:1-21](file://prisma/migrations/20260707210000_staff_login_account_field/migration.sql#L1-L21)
- [src/purely-profit/member/platform-membership/store-sub-account-login.service.ts:271-359](file://src/purely-profit/member/platform-membership/store-sub-account-login.service.ts#L271-L359)
- [src/purely-profit/auth/auth-account-lookup.service.ts:416-448](file://src/purely-profit/auth/auth-account-lookup.service.ts#L416-L448)

章节来源
- [prisma/migrations/20260707210000_staff_login_account_field/migration.sql:1-21](file://prisma/migrations/20260707210000_staff_login_account_field/migration.sql#L1-L21)
- [src/purely-profit/member/platform-membership/store-sub-account-login.service.ts:1-261](file://src/purely-profit/member/platform-membership/store-sub-account-login.service.ts#L1-L261)
- [src/purely-profit/auth/auth-account-lookup.service.ts:410-561](file://src/purely-profit/auth/auth-account-lookup.service.ts#L410-L561)
- [src/purely-profit/auth/auth.utils.ts:121-138](file://src/purely-profit/auth/auth.utils.ts#L121-L138)
...
SlotSvc --> PM
ReadSvc --> PM
LoginSvc --> Utils["AuthUtils"]
LoginSvc --> DB["PrismaService"]
```
...
- **新增** 登录账号查找性能优化：通过独立的 loginAccount 字段直接查询，避免了复杂的邮箱格式解析，显著提升登录响应速度。
...
- **新增** loginAccount 字段相关问题
  - 检查数据库迁移是否成功执行，确认 login_account 字段存在且有适当索引。
  - 验证 loginAccount 格式是否符合要求（6-32位字母数字下划线，非保留账号）。
  - 确认全局唯一性预检逻辑正确执行，避免跨门店冲突。
  - 参考路径：[prisma/migrations/20260707210000_staff_login_account_field/migration.sql:1-21](file://prisma/migrations/20260707210000_staff_login_account_field/migration.sql#L1-L21)
...
## 结论
本系统通过"装饰器 + 守卫 + 服务"的分层架构，实现了对多门店场景下子账户的精细化管控：从创建、激活到权限分配均有明确边界与强一致的事务保障；通过子账号封禁守卫与权限守卫，确保主账号专属能力不被子账号越权访问；通过审计与登录安全机制，提升整体安全性与可追溯性。**最新增强**：财务子账户现已获得完整的交接班管理能力，包括查看、创建和更新交接班记录的权限，这大大提升了财务人员在日常运营中的工作效率。**重大重构**：员工认证系统引入了独立的 loginAccount 字段，消除了 email 字段的多语义编码问题，显著提升了登录性能和数据完整性。建议在各业务模块中统一使用装饰器与守卫，并遵循配额与功能开关前置校验的流程，以获得稳定一致的用户体验与安全基线。
...
- 最佳实践
  ...
  - **新增** 使用 loginAccount 字段时应遵循新的验证规则，避免与现有手机号格式冲突。
- 常见问题
  ...
  - **新增** loginAccount 字段冲突：检查全局唯一性预检逻辑，确认跨门店账号唯一性。
  - **新增** 登录性能问题：确认 loginAccount 字段索引正常，优先使用直接字段查询而非邮箱格式解析。