...
- [src/purely-profit/dashboard/business-analysis/business-analysis.utils.ts](file://src/purely-profit/dashboard/business-analysis/business-analysis.utils.ts)
...
- [src/purely-profit/operations/suppliers/suppliers.controller.ts](file://src/purely-profit/operations/suppliers/suppliers.controller.ts)
- [src/purely-profit/operations/suppliers/suppliers.service.ts](file://src/purely-profit/operations/suppliers/suppliers.service.ts)
- [src/purely-profit/operations/suppliers/suppliers-read.service.ts](file://src/purely-profit/operations/suppliers/suppliers-read.service.ts)
- [src/purely-profit/operations/suppliers/suppliers-write.service.ts](file://src/purely-profit/operations/suppliers/suppliers-write.service.ts)
...
## 更新摘要
**变更内容**
- 业务分析模块时区处理已修复，新增上海时区支持确保日报表与业务期望一致
- 采购管理查询的排序确定性已更新，供应商查询现在包含明确的排序保证
...
- 供应商（Suppliers）：供应商信息管理。
- 控制器与服务：sales-record.controller.ts/sales-record.service.ts、costs.controller.ts/costs.service.ts、handover.controller.ts/handover.service.ts、spaces.controller.ts/spaces.service.ts、purchases.controller.ts/purchases.service.ts、suppliers.controller.ts/suppliers.service.ts。
...
- 典型接口（示意）：
  ...
  - 供应商：GET /operations/suppliers/list、POST /operations/suppliers、PUT /operations/suppliers/:id

#### 采购管理查询优化

**更新** 采购管理查询的排序确定性已增强，供应商查询现在包含明确的排序保证。

- **进货单查询排序**：`queryPurchaseOrders` 函数现在使用确定的排序规则 `orderBy: [{ date: 'desc' }, { id: 'desc' }]`，确保分页查询结果的稳定性。
- **供应商统计排序**：`countPurchaseSuppliers` 函数在查询供应商时使用 `orderBy: { supplierId: 'asc' }`，保证供应商计数的确定性。
...
participant Query as "PurchasesQuery"
participant DB as "Prisma"
C->>Ctrl : "GET /operations/purchases/list?page=1"
Ctrl->>Svc : "list(query)"
Svc->>Query : "queryPurchaseOrders(params)"
Query->>DB : "findMany({ orderBy : [{ date : 'desc' }, { id : 'desc' }] })"
DB-->>Query : "确定顺序的结果集"
Query-->>Svc : "分页结果"
Svc-->>Ctrl : "标准化响应"
Ctrl-->>C : "200 OK"
...
### 经营分析模块（Business Analysis）

**更新** 经营分析模块时区处理已修复，新增上海时区支持确保日报表与业务期望一致。

- **上海时区支持**：新增 `getShanghaiDayStartMs` 和 `formatShanghaiDayLabel` 函数，确保时间戳计算与 SQL 中的 `date_trunc('day', col + interval '8 hours') - interval '8 hours'` 完全对齐。
- **日趋势聚合**：所有日期计算不再依赖 Node.js 进程本地时区，确保跨环境一致性。
- **报表导出**：CSV 导出功能使用统一的上海时区处理，保证导出数据与业务期望一致。

```mermaid
sequenceDiagram
participant C as "客户端"
participant Ctrl as "BusinessAnalysisController"
participant Svc as "BusinessAnalysisService"
participant Utils as "BusinessAnalysisUtils"
participant Mapper as "BusinessAnalysisMapper"
participant DB as "Prisma"
C->>Ctrl : "GET /business-analysis?period=today"
Ctrl->>Svc : "getAnalysis(query)"
Svc->>Utils : "resolveCurrentRange(now)"
Utils->>Utils : "getShanghaiDayStartMs(timestamp)"
Utils-->>Svc : "上海时区开始时间"
Svc->>DB : "fetchBusinessAnalysisMetrics(range)"
DB-->>Svc : "原始指标数据"
Svc->>Mapper : "buildDailyTrend(start, end)"
Mapper->>Mapper : "formatShanghaiDayLabel(day)"
Mapper-->>Svc : "格式化后的趋势数据"
Svc-->>Ctrl : "完整分析报告"
Ctrl-->>C : "200 OK"
```
...
- **时区优化**：经营分析模块使用固定的上海时区偏移量（UTC+8），避免依赖系统时区配置，提升跨环境一致性。
- **排序确定性**：采购管理查询使用复合排序条件（日期+ID），确保分页结果稳定性，避免大数据量下的排序抖动。
...
- **时区问题**：经营分析报表时间显示异常时，检查服务器时区配置，但系统已内置上海时区处理，不应依赖系统配置。
- **排序不稳定**：采购管理分页结果不一致时，检查是否使用了正确的排序条件，系统已确保使用 `{ date: 'desc' }, { id: 'desc' }` 的复合排序。
...
## 结论
本系统围绕多租户架构设计，通过清晰的模块边界、严格的权限控制与完善的读写分离，实现了门店、会员、员工、商品、财务、营销、运营等核心业务的 API 覆盖。本次更新重点增强了经营分析模块的时区处理准确性和采购管理查询的排序确定性，进一步提升了系统的稳定性和用户体验。建议在生产环境中配合缓存、索引与并发控制策略，持续优化查询与写入性能，并完善监控与审计日志以保障数据一致性与可追溯性。
...
- **时区处理规范**：所有时间相关功能使用 UTC+8（上海时区）进行处理，不依赖 Node.js 进程本地时区配置。
- **排序稳定性保证**：所有分页查询都使用复合排序条件，确保大数据量下的结果稳定性。
