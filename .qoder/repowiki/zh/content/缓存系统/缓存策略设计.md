...
## 引言
本文件围绕缓存策略设计展开，系统性阐述缓存键命名规范、缓存键生成算法与管理策略；详述缓存预热配置参数、预热策略选择与优先级排序；并给出缓存键冲突处理、键过期时间设置与键空间管理的最佳实践。文档同时结合代码库中的实际实现，提供可操作的设计建议与落地方法，帮助在不同业务场景下选择合适的缓存策略。

## 项目结构
本项目的缓存相关能力集中在 Redis 模块中，围绕“键定义、预热配置、执行器、失效器、服务封装”等模块协同工作，并通过可观测性模块对缓存预热进行上下文与高亮指标统计。

```mermaid
graph TB
subgraph "Redis 缓存层"
CK["cache-keys.ts<br/>缓存键定义"]
...
## 核心组件
- 缓存键定义：集中管理所有业务键的命名规范与生成规则，确保全局一致性与可维护性。
...
## 架构总览
缓存策略由“键定义—配置—执行—服务—失效—观测”闭环构成。预热服务作为中枢，负责解析配置、调用执行器批量生成与写入键值，同时记录上下文与高亮指标；失效器在数据变更时触发清理；Redis 封装提供稳定的数据通道。

```mermaid
sequenceDiagram
participant Svc as "预热服务"
participant Exec as "预热执行器"
participant Keys as "缓存键定义"
...
## 结论
通过统一的键命名规范、严谨的预热配置与执行编排、完善的失效与清理策略，以及可观测性的全程覆盖，本项目实现了可扩展、可维护、高性能的缓存体系。建议在实际业务中遵循“前缀隔离、版本化、差异化 TTL、批量写入、持续观测”的原则，结合自身流量特征与数据特性，迭代优化缓存策略。

## 附录
- Redis 模块装配与客户端封装，确保缓存层的稳定与可替换性。
- 参考路径：[redis.module.ts](file://src/redis/redis.module.ts)、[redis.service.ts](file://src/redis/redis.service.ts)