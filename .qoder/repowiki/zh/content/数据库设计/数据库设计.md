...
- 实体与字段要点
  ...
  - **新增 WeChat 认证字段**：wechatOpenid（@unique）、wechatUnionid、wechatAvatar、wechatNickname、wechatPhone。
- 主键/外键
...
- 索引策略
  ...
  - **新增**：wechatOpenid（唯一索引）、wechatUnionid（普通索引）、wechatPhone（普通索引）。
- 约束与校验
  - 唯一性：users.email、users.id_number、stores.owner_id、staff.user_id、staff.email、**wechatOpenid**。
...
**更新** 新增 WeChat 认证相关字段，支持微信第三方登录和用户信息管理
...
- **新增 WeChat 认证字段**
  ...
  - wechatPhone：微信绑定手机号，存储用户微信绑定的手机号码。
- **迁移文件**
...
**新增** WeChat 认证相关字段，支持微信第三方登录和用户信息管理

**章节来源**
- [store-accounts.prisma:36-44](file://prisma/purely-profit/stores/store-accounts.prisma#L36-L44)
- [20260615120000_add_club_wechat_auth_fields/migration.sql](file://prisma/migrations/20260615120000_add_club_wechat_auth_fields/migration.sql)
- [20260615130000_add_wechat_phone_to_users/migration.sql](file://prisma/migrations/20260615130000_add_wechat_phone_to_users/migration.sql)
...
- 索引策略
  ...
  - 单列索引：phone、email、id_number、expiresAt、dueDate、stock、prepaidAmount、**wechatOpenid、wechatUnionid、wechatPhone** 等热点字段。
  - 搜索索引：TRGM 索引支持模糊搜索，提升文本检索性能。
  - 时间序列：大量按 createdAt 查询的表均建立相应索引。
  - **新增 WeChat 字段索引**：wechatOpenid（唯一索引）、wechatUnionid（普通索引）、wechatPhone（普通索引）。
- 查询优化
  - 使用范围查询时尽量结合 storeId 与时间字段，减少全表扫描。
  - 对枚举字段进行过滤时，利用索引与枚举类型减少回表。
  - 新增库存与预付费字段查询支持索引优化。
  - **微信认证查询优化**：基于 openid 和 unionid 的快速用户识别。
- 写入优化
  - 批量写入销售明细与库存日志时，注意事务边界与索引维护成本。
  - 营销产品库存更新采用原子操作，避免竞态条件。
  - **微信认证字段写入优化**：避免重复的微信用户标识冲突。
- 缓存策略
  - 建议对热点维度（如会员等级、员工状态、门店订阅状态、产品库存、**微信用户标识**）引入 Redis 缓存，设置合理 TTL。
...
## 故障排查指南
- 常见错误与定位
  - 唯一约束冲突：用户邮箱、身份证、门店所有者、员工工号、子账号槽位、**微信 openid** 等。
...
## 结论
该数据库设计以 Store 为核心上下文，围绕会员、营销、财务、运营、人事等模块构建完整闭环。通过枚举类型、复合索引与外键约束确保数据一致性与查询效率。新增的营销产品库存字段和空间会话预付费字段进一步完善了业务功能。搜索索引优化提升了文本检索性能。**新增的 WeChat 认证相关字段为系统提供了完整的第三方登录能力，包括用户身份识别、头像昵称展示和手机号绑定等功能**。建议在生产环境中配套缓存、分页优化与监控告警体系，持续迭代索引与查询模式，保障高并发下的稳定性与性能。
...
- **微信认证场景**：基于 openid 的快速用户识别，基于 unionid 的跨应用用户识别。

### 示例数据（示意）
- 用户：邮箱唯一，实名与身份证唯一，**微信 openid 唯一**。
...
- **微信认证**：openid 唯一标识，unionid 跨应用识别，头像昵称完整，手机号绑定。
...
- **微信认证数据**：用户微信信息定期同步更新，openid 和 unionid 长期保留用于用户识别。
...
- 新增迁移：营销产品库存字段、空间会话预付费字段、搜索索引优化、**WeChat 认证字段**。
- **迁移顺序**：先添加 WeChat 认证字段，再建立相应索引。
...
- **微信认证安全**：openid 和 unionid 属于敏感用户标识，需实施严格的访问控制和加密存储。
