...
- [voucher-orders.prisma](file://prisma/purely-profit/club/voucher-orders.prisma)
...
- [20260810000000_add_club_voucher_order/migration.sql](file://prisma/migrations/20260810000000_add_club_voucher_order/migration.sql)
- [20260810140000_add_voucher_order_breakdown_items/migration.sql](file://prisma/migrations/20260810140000_add_voucher_order_breakdown_items/migration.sql)
- [20260810150000_add_marketing_product_billing_config/migration.sql](file://prisma/migrations/20260810150000_add_marketing_product_billing_config/migration.sql)
</cite>

## 更新摘要
**变更内容**
- 新增团购券订单表（ClubVoucherOrder）及其相关索引和约束
- 新增营销产品计费配置字段，支持服务产品和团购券类型
- 扩展产品模型以支持团购券业务场景
- 新增订单优惠拆解快照功能
- 更新空间会话预付费字段以支持团购券核销
...
A --> F["marketing.products.prisma<br/>营销产品/库存/计费配置"]
...
A --> N["club.voucher-orders.prisma<br/>团购券订单"]
```
...
- [products.prisma:1-63](file://prisma/purely-profit/marketing/products.prisma#L1-L63)
...
- [voucher-orders.prisma:1-86](file://prisma/purely-profit/club/voucher-orders.prisma#L1-L86)
...
- 领域模型：用户/门店/员工/订阅、会员与积分/豆/充值流水、营销客户/储值/消费/积分流水、营销活动、营销产品/库存/计费配置、销售订单/明细、库存调整日志、财务往来账、员工/部门/岗位/排班/薪资、子账号/交接班/附加项、平台会员计划/订单/积分流水、空间管理/会话/预付费、团购券订单。
- 迁移演进：从初始用户表到门店、订阅、会员、合作伙伴与提现等模块逐步扩展，新增营销产品库存管理和空间会话预付费功能，以及 WeChat 认证相关字段，最新新增团购券订单表和营销产品计费配置。
...
- [products.prisma:11-63](file://prisma/purely-profit/marketing/products.prisma#L11-L63)
...
- [voucher-orders.prisma:2-86](file://prisma/purely-profit/club/voucher-orders.prisma#L2-L86)
...
STORES ||--o{ CLUB_VOUCHER_ORDERS : "团购券订单"
...
MARKETING_PRODUCTS ||--o{ CLUB_VOUCHER_ORDERS : "团购券商品"
...
CLUB_VOUCHER_ORDERS ||--o{ SPACE_SESSIONS : "开台核销"
```
...
- 实体与字段要点
  ...
  - **新增团购券支持**：type（service/voucher）、validDays（有效天数）、billingMode（计费方式）、hourlyRate（计时单价）、countdownMinutes（预设时长）、countdownPrice（台位费）、autoCheckout（自动结账）。
- 主键/外键
...
- 约束与校验
  ...
  - **新增**：type 枚举限制为 service 或 voucher，billingMode 限制为 items/timed/mixed/countdown。
- 业务规则
  - 库存不足时禁止销售；支持库存预警与补货提醒。
  - 产品状态影响展示与销售权限。
  - **团购券商品支持多种计费模式和有效期管理**。

**更新** 新增营销产品库存字段和团购券支持，支持库存管理和销售控制，扩展产品模型以支持服务产品和团购券类型
...
- [20260810150000_add_marketing_product_billing_config/migration.sql:1-8](file://prisma/migrations/20260810150000_add_marketing_product_billing_config/migration.sql#L1-L8)
...
### 团购券订单（Club Voucher Orders）
- 实体与字段要点
  - ClubVoucherOrder：团购券码、平台标识、购买门店、用户ID、顾客ID、商品ID、商品名称快照、价格信息、购买数量、到店人数、顾客信息、订单号、金额信息、优惠拆解、积分抵扣、支付渠道、交易单号、订单状态、有效期、核销时间、退款信息等。
  - **新增优惠拆解快照**：breakdownItems JSON 字段，存储下单时后端生成的优惠明细展示行。
- 主键/外键
  - Order.storeId → Store；userId → User；customerId → MarketingCustomer；productId → MarketingProduct。
- 索引策略
  - (storeId, status, createdAt DESC)、(userId, status, createdAt DESC)、usedSessionId。
  - 唯一索引：voucher_code、order_no、refund_no。
- 约束与校验
  - 金额字段使用整数（分）；状态枚举化；券码和订单号唯一性约束。
- 业务规则
  - 团购券订单支持未支付、待使用、已使用、已退款、已过期等状态流转。
  - 支持开台核销和退款流程，与空间会话关联。
  - **优惠拆解快照提供详细的优惠明细展示**。

**更新** 新增团购券订单表，支持团购券商品的完整生命周期管理

**章节来源**
- [voucher-orders.prisma:2-86](file://prisma/purely-profit/club/voucher-orders.prisma#L2-L86)
- [20260810000000_add_club_voucher_order/migration.sql:1-74](file://prisma/migrations/20260810000000_add_club_voucher_order/migration.sql#L1-L74)
- [20260810140000_add_voucher_order_breakdown_items/migration.sql:1-3](file://prisma/migrations/20260810140000_add_voucher_order_breakdown_items/migration.sql#L1-L3)
...
Store --> Voucher["ClubVoucherOrder"]
...
- 索引策略
  ...
  - **新增团购券订单索引**：store_id + status + created_at 复合索引、user_id + status + created_at 复合索引、used_session_id 索引。
- 查询优化
...
- 缓存策略
  ...
  - **团购券订单缓存**：券码解析结果、订单状态等可缓存以减少数据库压力。
- 分页与分片
...
- **团购券订单场景**：基于券码的快速订单定位，基于门店和用户维度的列表查询，基于会话的反向追溯。
...
- 营销产品：名称唯一，库存非负，默认 0，**支持服务商品和团购券商品类型**。
...
- **团购券订单**：券码唯一，订单号唯一，支持完整的状态流转和退款流程。
...
- **团购券订单数据**：订单数据按状态和有效期进行清理，已使用订单保留 1 年用于对账，过期订单及时清理。
...
- 新增迁移：营销产品库存字段、空间会话预付费字段、搜索索引优化、**WeChat 认证字段、微信电话唯一索引约束、用户最后活跃时间字段、团购券订单表、营销产品计费配置**。
- **迁移顺序**：先添加 WeChat 认证字段，再建立相应索引，然后添加微信电话唯一索引约束，最后添加用户最后活跃时间字段和团购券订单表。
...
- **团购券订单安全**：券码和订单号具有唯一性约束，防止重复使用和伪造；订单状态流转需要严格的权限控制。