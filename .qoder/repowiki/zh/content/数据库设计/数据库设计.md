...
A --> F["operations.sales-records.prisma<br/>销售订单/明细"]
A --> G["goods.inventory.prisma<br/>库存调整日志"]
A --> H["finance.accounts.prisma<br/>往来账"]
A --> I["staff.employees.prisma<br/>员工/部门/岗位/排班/薪资"]
A --> J["operations.handover.prisma<br/>子账号/交接班/附加项"]
A --> K["member.platform-membership.prisma<br/>平台会员计划/订单/积分流水"]
```
...
- 领域模型：用户/门店/员工/订阅、会员与积分/豆/充值流水、营销客户/储值/消费/积分流水、营销活动、销售订单/明细、库存调整日志、财务往来账、员工/部门/岗位/排班/薪资、子账号/交接班/附加项、平台会员计划/订单/积分流水。
- 迁移演进：从初始用户表到门店、订阅、会员、合作伙伴与提现等模块逐步扩展。
...
PRODUCTS ||--o{ SALE_ORDER_ITEMS : "被购买"
...
- 索引策略
  ...
  - 单列索引：phone、email、id_number、expiresAt、dueDate 等热点字段。
...
- 缓存策略
  - 建议对热点维度（如会员等级、员工状态、门店订阅状态）引入 Redis 缓存，设置合理 TTL。
...
- 排查步骤
  ...
  - 核对业务状态机（如提现状态、订单状态）是否符合预期。
- 修复建议
  - 补建缺失索引；调整查询谓词顺序以命中索引。
  - 在应用层增加幂等性与重试机制，避免重复提交。

## 结论
该数据库设计以 Store 为核心上下文，围绕会员、营销、财务、运营、人事等模块构建完整闭环。通过枚举类型、复合索引与外键约束确保数据一致性与查询效率。建议在生产环境中配套缓存、分页优化与监控告警体系，持续迭代索引与查询模式，保障高并发下的稳定性与性能。

## 附录

### 数据访问模式
- 读多写少场景：会员列表、营销客户列表、销售订单明细、财务账单等，优先使用复合索引与缓存。
...
- 访问控制：基于角色的权限矩阵（RBAC），限制对会员、财务、人事等敏感模块的访问。
