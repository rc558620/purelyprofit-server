...
} from '../../../dashboard/business-analysis/dto/business-analysis-query.dto';

// ─────────────────────────────────────────────────────────────
// 总览时间周期枚举（Boss 视角用 today/week/month/year）
// ─────────────────────────────────────────────────────────────
...
export class GetPulseDashboardOverviewQueryDto {
  ...
  @ApiPropertyOptional({
    example: 1,
    description: '指定门店 ID（单店模式），不传则汇总所有门店',
  })
  @IsOptional()
 ...
 * 经营分析（复用 BusinessAnalysis 参数，但 storeId 限定为老板门店）
 */
export class GetPulseDashboardAnalysisQueryDto {
  ...
  @ApiPropertyOptional({
    example: 1,
    description: '门店 ID（只能是老板自有门店），不传时仅支持有单一门店的老板',
  })
  @IsOptional()
  @Transform(transformOptionalInt)
  @IsInt({ message: '门店 ID 必须是整数' })
