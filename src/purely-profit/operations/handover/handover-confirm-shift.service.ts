...
export class HandoverConfirmShiftService {
  constructor(
    ...
    private readonly fallbackService: HandoverConfirmShiftFallbackService,
  ) {}
  ...
  async findSourceShiftRecord(
    ...
    if (!matched && typeof options.shiftReferenceAt !== 'number') {
      ...
      if (extendedShifts.length > 0) {
        return this.fallbackService.findEarliestUnhandedShift(
          storeId,
          extendedShifts,
          options,
        );
  ...
  async ensureShiftNotHandedOver(
    ...
    if (shiftRecord?.employeeId) {
      ...
      where = {
        ...
        OR: buildShiftMatchConditions(shiftRecord, handoverAt),
      };
    } else if (shiftRecord) {
  ...
  private async findNextShiftRecord(
    ...
    const nextByEndTime = allShifts.find((shift) => {
      if (isSameShiftRecord(shift, currentShiftRecord)) return false;
