...
describe('ScanOrderingTableClearService', () => {
  let service: ScanOrderingTableClearService;
  ...
  beforeEach(async () => {
    ...
    service = module.get(ScanOrderingTableClearService);
  });
  ...
  it('扫码会话与手工单混合：已出餐手工单一并置为已完成', async () => {
    ...
    // 手工单更新与扫码会话订单更新各一次
    const manualEntryUpdate = transaction.scanOrders.updateMany.mock.calls.find(
  ...
  it('桌台不存在时拒绝清桌', async () => {
    transaction.scanOrderingTable.findFirst.mockResolvedValue(null);
