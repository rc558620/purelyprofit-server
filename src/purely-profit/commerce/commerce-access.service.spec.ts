...
describe('CommerceAccessService', () => {
  ...
  it('getManageableStoreId 在当前 membership 有权限时返回 storeId', async () => {
    ...
    await expect(service.getManageableStoreId(user, 'goods:view')).resolves.toBe(18);
  });

  it('getManageableStoreId 在当前 membership 无权限时返回 null', async () => {
    await expect(service.getManageableStoreId(user, 'goods:view')).resolves.toBeNull();
  });

  it('resolveSingleStoreId 在无权限时抛出异常', async () => {
    await expect(
      service.resolveSingleStoreId(user, undefined, 'goods:view', '无权查看该门店商品'),
  ...
  it('ensureCanAccessStore 在当前 membership 门店不匹配时抛出异常', async () => {
    ...
    await expect(
      service.ensureCanAccessStore(user, 18, 'goods:update', '无权操作该门店商品'),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });
  ...
  it('findOperatorStaffIdForStore 在当前 membership 命中时返回 staffId', async () => {
    ...
    await expect(service.findOperatorStaffIdForStore(user, 18)).resolves.toBe(55);
  });

  it('findOperatorStaffIdForStore 在当前 membership 未命中时返回 null', async () => {
    await expect(service.findOperatorStaffIdForStore(user, 18)).resolves.toBeNull();
  });
});
