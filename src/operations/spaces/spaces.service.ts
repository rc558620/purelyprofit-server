...
export class SpacesService {
    ...
    const updated = await this.prisma.$transaction(async (transaction) => {
      const nextStatus =
        dto.status === PrismaSpaceStatus.cleaning
          ...
          : await this.resolveReservationBackStatus(transaction, space.id);
  ...
  async openSpaceSession(
    ...
    const session = await this.prisma.$transaction(async (transaction) => {
      if (space.status === PrismaSpaceStatus.reserved) {
        const latestPendingReservation = await transaction.spaceReservation.findFirst({
          where: {
            spaceId: space.id,
            status: PrismaSpaceReservationStatus.pending,
          },
  ...
  private ensureOpenSessionPayload(
    ...
    if (payload.guestPhone && !this.isValidContact(payload.guestPhone)) {
      ...
      if (payload.autoCheckout) {
        ...
        const prepaidSettlementChannel =
          ...
          (prepaidCustomerPaymentMethod === 'groupon_voucher'
            ? this.resolveVoucherSettlementChannel(prepaidVoucherPlatform)
            : 'direct_cashier');
  ...
  private parseNumericProductId(productId: string): number {
    const parsed = Number.parseInt(productId, 10);
    if (!Number.isInteger(parsed) || parsed <= 0) {
      throw new BadRequestException('商品 ID 不合法');
    }
    return parsed;
  }
  ...
  private resolveVoucherSettlementChannel(
    voucherPlatform?: string,
  ): SpaceSettlementChannelValue {
    const normalized = voucherPlatform?.trim().toLowerCase();
    if (!normalized) {
