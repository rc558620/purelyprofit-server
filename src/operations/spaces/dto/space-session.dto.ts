...
export type SpaceTimeFeeModeValue =
  (typeof SPACE_TIME_FEE_MODE_VALUES)[number];
...
export class OpenSpaceSessionDto {
  ...
  @ApiPropertyOptional({ example: 'groupon_voucher', description: '预付顾客支付方式', enum: SPACE_CUSTOMER_PAYMENT_METHOD_VALUES })
  @IsOptional()
  ...
  @ApiPropertyOptional({ example: 'meituan_groupon', description: '预付结算渠道', enum: SPACE_SETTLEMENT_CHANNEL_VALUES })
  @IsOptional()
...
export class CheckoutSpaceSessionPreviewSummaryDto {
  ...
  @ApiPropertyOptional({
    example: 'timed',
    description: '兼容旧版前端的倒计时台位费口径：timed=按实际计时，fixed=按固定台位费',
    enum: SPACE_COUNTDOWN_FEE_MODE_VALUES,
  })
  countdownFeeMode?: SpaceCountdownFeeModeValue;
}
