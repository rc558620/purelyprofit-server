...
export const CLUB_RECORD_FILTER_VALUES = ['all', 'recharge', 'consume'] as const;
...
export class ListClubRecordsQueryDto {
  @ApiPropertyOptional({
    ...
    description: '流水筛选类型：all=全部 recharge=充值 bonus consume=消费 refund',
  })
  @IsOptional()
...
export class ClubRecordDto {
  ...
  @ApiProperty({ example: 500, description: '展示金额，单位元；收入为正，支出为负' })
  amount: number;
  ...
  @ApiPropertyOptional({ example: 'purelyClub · 望京旗舰店', description: '门店名称' })
  @IsOptional()
