...
describe('ClubStoreAccessService', () => {
  ...
  beforeEach(async () => {
    ...
    prismaService.storeInviteCode.findMany.mockResolvedValue([
      { code: 'AB23CD45', storeId: 18 },
    ]);
    prismaService.store.findFirst.mockResolvedValue(storeRecord);
  ...
  describe('resolveScanCode', () => {
    ...
    it('无法识别的扫码内容返回 not_found', async () => {
      ...
      expect(prismaService.store.findFirst).not.toHaveBeenCalled();
    });
  ...
  describe('resolvePublicInviteEntry', () => {
    ...
    it('渠道二维码（带 token）扫码成功后 scanCount 递增', async () => {
      prismaService.storeInviteQrIssue.findUnique.mockResolvedValue({
        id: 1,
        status: 'active',
        storeId: 18,
        scanCount: 5,
      ...
      expect(prismaService.storeInviteQrIssue.update).toHaveBeenCalledWith({
        where: { id: 1 },
    ...
    it('已撤销的渠道二维码返回停用提示且不递增 scanCount', async () => {
      ...
      expect(prismaService.storeInviteQrIssue.update).not.toHaveBeenCalled();
      expect(result.status).toBe('inactive');
      expect(result.store).toBeNull();
