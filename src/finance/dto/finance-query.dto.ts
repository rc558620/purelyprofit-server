...
export class FinanceReportQueryDto {
  @ApiPropertyOptional({
    enum: ['today', 'week', 'month', 'quarter', 'year', 'custom_month', 'custom_range'],
    description: '报表中心财务周期筛选',
  })
  @IsOptional()
  @IsIn(['today', 'week', 'month', 'quarter', 'year', 'custom_month', 'custom_range'], {
    message: '财务报表周期不合法',
  })
  period?:
    | 'today'
  ...
  @ApiPropertyOptional({ example: 1747180800000, description: '按月模式对应的单日时间戳（ms）' })
  @IsOptional()
  @Type(() => Number)
  ...
  @ApiPropertyOptional({ example: 1746057600000, description: '自定义区间开始时间戳（ms）' })
  @IsOptional()
  @Type(() => Number)
  ...
  @ApiPropertyOptional({ example: 1748735999999, description: '自定义区间结束时间戳（ms）' })
  @IsOptional()
  @Type(() => Number)
